...
class LLMEngine:
    ...
    def add_request(self, inputs_embeds: list[torch.Tensor], sampling_params: SamplingParams | list[SamplingParams]):
        ...
        for inputs_embeds, sampling_params in zip(inputs_embeds, sampling_params):
            seq = Sequence([], input_embeds=inputs_embeds, sampling_params=sampling_params)
            self.scheduler.add(seq)
    ...
    def generate(
        ...
        while not self.is_finished():
            ...
            for seq_id, token_ids, hidden_states in output:
                outputs[seq_id] = token_ids, hidden_states
                if use_tqdm:    
                    pbar.update(1)
        outputs = [outputs[seq_id] for seq_id in sorted(outputs.keys())]
        outputs = [{"token_ids": token_ids, "hidden_states": hidden_states} for token_ids, hidden_states in outputs]
        if use_tqdm:
            pbar.close()
        return outputs
