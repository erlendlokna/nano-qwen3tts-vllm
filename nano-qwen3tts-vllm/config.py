...
def _validate_layer_types(layer_types):
    ...
    Raises:
        ValueError: If layer types are invalid
    """
    ...
    for i, layer_type in enumerate(layer_types):
        if layer_type not in valid_types:
            raise ValueError(
                f"Invalid layer type '{layer_type}' at index {i}. "
...
def _validate_rope_config(config):
    ...
    Args:
        config: Config object with rope_scaling attribute
        
    Raises:
        ValueError: If RoPE config is invalid
    """
    ...
    if rope_type not in valid_rope_types:
        raise ValueError(
            ...
            f"Must be one of {valid_rope_types}"
        )
    
    # Validate rope_type-specific parameters
    if rope_type in {"dynamic", "longrope", "llama3"}:
        if "original_max_position_embeddings" not in rope_scaling:
            raise ValueError(
                f"rope_type '{rope_type}' requires 'original_max_position_embeddings'"
            )


class Qwen3TTSSpeakerEncoderConfig(BaseConfig):
    r"""
...
class Qwen3TTSTalkerCodePredictorConfig(BaseConfig):
    r"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        _validate_layer_types(self.layer_types)
        self.num_code_groups = num_code_groups


class Qwen3TTSTalkerConfig(BaseConfig):
    r"""
    ...
    def __init__(
        ...
        attention_dropout=0,
        head_dim=None,
        num_code_groups=16,  # Total codebooks: 1 (talker) + 15 (predictor) = 16
        text_vocab_size=151936,  # NEW: Qwen2 tokenizer vocab size
        text_hidden_size=2048,   # Text embedding dimension
